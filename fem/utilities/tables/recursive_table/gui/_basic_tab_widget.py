...
from ._recursive_table import RecursiveTable
...
class _CommandDispatcher(CommandDispatcher):
    ...
    def _wrap_command(self, command):
        command = _ChildCommand(command, self.tab_widget)

        # debuginfo('wrap command')
...
class BasicTabWidget(QtWidgets.QTabWidget):
    ...
    def add_table(self, table, name):
        ...
        table.data_changed.connect(self._data_changed)
