...
class BaseCommand(Command):
    ...
    def __init__(self, action):
        ...
        self.table = self.main_window.table_1
...
class AddAction(BaseAction):
    ...
    def _redo(self):
        self._removed_data = self.main_data.add(self._removed_data)
        return True
    ...
    def _undo(self):
        self.main_data.remove(len(self.main_data) - 1)
...
class RemoveAction(BaseAction):
    ...
    def _redo(self):
        ...
        if len(selection) == 0:
            selection = [(len(self.main_data) - 1, 0)]
            self.action_data.selection = selection

        for index in selection:
            tmp = self.main_data.remove(index[0])
            if tmp is not None:
                self._removed_data.append(tmp)
    ...
    def _undo(self):
        ...
        for i in range(len(removed_indices)):
            ...
            self.main_data.insert(index[0], data)
...
class InsertAction(BaseAction):
    ...
    def _redo(self):
        self.main_data.insert(self.action_data.index[0])
        return True
    ...
    def _undo(self):
        self.main_data.remove(self.action_data.index[0])
...
class SetDataActionData(ActionData):
    def __init__(self, index, value, enter_down=False):
...
class SetDataAction(BaseAction):
    ...
    def __init__(self, action_data):
        ...
        data = self.main_data[self.action_data.index[0]]
    ...
    def _redo(self):
        result = self.main_data.set_data(self.action_data.index, self.action_data.value)
        return result[0]
    ...
    def _undo(self):
        ...
        self.main_data[self.action_data.index[0]].load(self.old_data)
...
class UpAction(BaseAction):
    ...
    def _redo(self):
        return self.main_data.up(self.action_data.index[0])
    ...
    def _undo(self):
        self.main_data.down(self.action_data.index[0] - 1)
...
class DownAction(BaseAction):
    ...
    def _redo(self):
        return self.main_data.down(self.action_data.index[0])
    ...
    def _undo(self):
        self.main_data.up(self.action_data.index[0] + 1)
...
class SetRowsAction(BaseAction):
    ...
    def _validate(self):
        ...
        current_rows = self.main_data.shape()[0]
    ...
    def _redo(self):
        ...
        if rows > current_rows:
            ...
            self._added_data = self.main_data.add_multiple(diff, self._added_data)

        else:
            diff = current_rows - rows
            self._removed_data = self.main_data.remove_multiple(diff)
    ...
    def _undo(self):
        if self._added_data is not None:
            self.main_data.remove_multiple(len(self._added_data))
        elif self._removed_data is not None:
            self.main_data.add_multiple(len(self._removed_data), self._removed_data)
