...
class MrSelectionModel(QtCore.QItemSelectionModel):
    def __init__(self, tree_view, model):
        super(MrSelectionModel, self).__init__(model)
    ...
    def select(self, selection, selection_flags):
        ...
        super(MrSelectionModel, self).select(selection, selection_flags)


class MrTreeModel(QtGui.QStandardItemModel):
    def __init__(self):
        super(MrTreeModel, self).__init__()


class MrTreeView(QtWidgets.QTreeView):
    ...
    def wrap_obj(cls, obj=None, *args, **kwargs):
        ...
        if obj is None:
            obj = MrTreeView(*args, **kwargs)
            return obj

        elif isinstance(obj, MrTreeView):
            return obj
        ...
        obj.__class__ = MrTreeView
        MrTreeView.__init__(obj, parent, False)

        return obj

    def __init__(self, parent=None, init=True):
        if init:
            super(MrTreeView, self).__init__(parent)

        self.setSelectionMode(QtWidgets.QTreeView.SingleSelection)

        self._model = MrTreeModel()
        ...
        self.selection_model = MrSelectionModel(self, self.model())
    ...
    def keyPressEvent(self, event):
        ...
        super(MrTreeView, self).keyPressEvent(event)
    ...
    def mousePressEvent(self, event):
        ...
        super(MrTreeView, self).mousePressEvent(event)

    def mouseReleaseEvent(self, event):
        self.right_button = False

        super(MrTreeView, self).mouseReleaseEvent(event)
...
if __name__ == '__main__':
    ...
    tree_view = MrTreeView.wrap_obj(QtWidgets.QTreeView())
